...
// ADD SERVICE REQUEST
router.post("/:id/request-service", async (req, res) => {
  try {
    ...
    user.requestedServices.push({ serviceId, description });
    await user.save();

    res.json({ message: "Service request submitted!", requested: user.requestedServices });
  } catch (err) {
