...
function ServiceRequestItem({ service }) {
  const serviceId = service._id || service.id; // <--- IMPORTANT FIX

  const [checked, setChecked] = useState(false);
  const [desc, setDesc] = useState("");

  if (!window._serviceRequests[serviceId]) {
    window._serviceRequests[serviceId] = { checked: false, desc: "" };
  }

  const toggle = () => {
    const newState = !checked;
    setChecked(newState);
    window._serviceRequests[serviceId].checked = newState;
  };

  const handleDescChange = (e) => {
    const text = e.target.value;
    setDesc(text);
    window._serviceRequests[serviceId].desc = text;
  };
